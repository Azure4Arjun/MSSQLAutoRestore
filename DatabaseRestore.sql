...
CREATE PROCEDURE [dbo].[DatabaseRestore]
      @Database NVARCHAR(4000), @RestoreDatabaseName NVARCHAR(MAX)= NULL, @BackupPath NVARCHAR(MAX), @MoveFiles NVARCHAR(MAX)= 'N', @MoveDataDrive
      NVARCHAR(MAX)= NULL, @MoveLogDrive NVARCHAR(MAX)= NULL, @TestRestore NVARCHAR(MAX)= 'N', @RunCheckDB NVARCHAR(MAX)= 'N', @LogToTable NVARCHAR(
      MAX)= 'N'
AS
     BEGIN
         SET NOCOUNT ON;
         ...
         IF @MoveFiles = 'Y'
             BEGIN
                 WITH Files
                      AS(SELECT ',MOVE '''+LogicalName+''' TO '''+CASE
                                                                      WHEN type = 'D'
                                                                      THEN @MoveDataDrive
                                                                      WHEN Type = 'L'
                                                                      THEN @MoveLogDrive
                                                                  END+REVERSE(LEFT(REVERSE(PhysicalName), CHARINDEX('\', REVERSE(PhysicalName), 1)-1))
                                                                  +'''' AS logicalcmds
             ...
             END;
         SET @cmd = 'RESTORE DATABASE '+@RestoreDatabaseName+' FROM DISK = '''+@FullCopyPath+@lastFullBackup+''' WITH NORECOVERY, REPLACE'+
         @MoveOption+CHAR(13);
         PRINT @Cmd;
         EXECUTE @cmd = [dbo].[CommandExecute] @Command = @cmd, @CommandType = 'RESTORE DATABASE', @Mode = 1, @DatabaseName = @database, @LogToTable
         = 'Y', @Execute = 'Y';
         ...
         SELECT @BackupCompletedstring = REPLACE(REPLACE(REPLACE(CONVERT( VARCHAR(19), CONVERT(DATETIME, BackupFinishDate, 112), 126), '-', ''), 'T',
         '_'), ':', '')
         FROM @headers;
         ...
         FOR SELECT backupFile
             ...
             WHERE backupFile LIKE '%.TRN'
                   ...
                   backupFile LIKE '%'+@Database+'%'
                   AND
                   RIGHT(backupFile, 19) > @BackupCompletedstring;
         OPEN backupFiles;  
         ...
         WHILE @@FETCH_STATUS = 0
             BEGIN
                 ...
                 --EXEC (@CMD)
                 EXECUTE @cmd = [dbo].[CommandExecute] @Command = @cmd, @CommandType = 'RESTORE LOG', @Mode = 1, @DatabaseName = @database,
         ...
         IF @RunCheckDB = 'Y'
             ...
             END;
         --If test restore then blow the database away (careful)
         IF @TestRestore = 'Y'
             BEGIN
